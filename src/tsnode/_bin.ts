...
import { TSError, VERSION, fileExists, getFile, parse, register } from './index';
...
function isFlagOnly(arg: string): boolean
{
    const name = arg.replace(/^--?/, '');

    // The value is part of this argument.
    if (/=/.test(name) || /^--no-/.test(arg)) return true;
...
if (argv.version)
{
    console.log(`tsos v${VERSION}`)
    console.log(`node ${process.version}`)
    process.exit(0);
}

if (argv.help) { console.log(`
Usage: tsos [options] [ -e script | script.ts ] [arguments]
...
Options:
  ...
  --visitors [path], --no-visitors Set visitor modules to perform extra transpilations
 ...
 * Evaluate a script.
 */
function evalAndExit(code: string, isPrinted: boolean): void
{
 ...
 */
function print(error: TSError): string
{
 ...
 */
function replEval(code: string, context: any, _filename: string, callback: (err?: Error, result?: any) => any): void
{
    let err: any;
    let result: any;
 ...
 */
function appendEval(input: string): () => void
{
 ...
 */
function getFileEval(path: string): string
{
 ...
 */
function fileExistsEval(path: string): boolean
{
 ...
 */
function isRecoverable(error: TSError): boolean
{
